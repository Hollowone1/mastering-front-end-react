...
const handleDelete = ()=> {
    ...
    return (
        <div className='article' style={{background: isEditing ? "a1869d":"ffffff"}}>
            <div className='card-header'>
                <h3>{article.author}</h3>
                <em>Posté le {dateFormater(article.data)}</em>
            </div>
            {
            ...
            <div className="btn-container">
                {
                    isEditing ? <button onClick={()=> handleEdit(false)}>Valider</button>:<button onClick={()=> setIsEditing(true)}>Edit</button>
                }
                <button onClick={()=>{
